...
import org.micro.pub.base.JacksonJsonMapper;
...
public class ServiceMsgConverter 
{
			...
			throws Exception
	{
		...
		for (int i = 0; i < paramTypesL; i++) 
		{
			...
			{
				if(ObjectCensor.isStrRegular(dataVal) && !"null".equals(dataVal))
				{
					if(dataVal.indexOf("null") != -1)
					{
						dataVal = dataVal.replace("null", "");
					}
				}
	...
	public static String beanToJsonConverter(Object obj) throws Exception 
	{
		...
		{
			serviceResult = obj.toString();
		}
		return URLEncoder.encode(serviceResult, "utf-8");
	}
