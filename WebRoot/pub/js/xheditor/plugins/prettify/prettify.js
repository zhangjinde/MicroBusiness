...
(function() {
	function y(b) {
		return b.replace(K, "&amp;").replace(L, "&lt;").replace(M, "&gt;")
	}
	function G(b, f, i) {
		switch (b.nodeType) {
			case 1 :
				...
				if (n) {
					...
					for (j = 0; j < n; ++j) {
						r = l[j];
						r.specified
								&& f.push(" ", r.name.toLowerCase(), '="',
										r.value.replace(K, "&amp;").replace(L,
												"&lt;").replace(M, "&gt;")
												.replace(W, "&quot;"), '"')
					}
				}
	...
	function B(b, f) {
		...
		var l = f.length, n = function(r) {
			for (var j = r.basePos, q = [j, z], m = 0, t = r.source.match(o)
					|| [], p = {}, c = 0, d = t.length; c < d; ++c) {
				...
				if (h) {
					h = e[1];
					var s = a.indexOf(h), u = s + h.length;
					if (e[2]) {
						u = a.length - e[2].length;
						s = u - h.length
					}
					k = k.substring(5);
					H(j + g, a.substring(0, s), n, q);
					H(j + g + s, h, P(k, h), q);
					H(j + g + u, a.substring(u), n, q)
				} else
					q.push(j + g, k)
			}
			r.decorations = q
	...
	function x(b) {
		...
		b.length
				&& i.push([Q, RegExp("^(?:" + b.replace(/\s+/g, "|") + ")\\b"),
						null]);
		f.push([z, /^\s+/, null, " \r\n\t\u00a0"]);
		i
				.push(
						[I, /^@[a-z_$][a-z_$@0-9]*/i, null],
						[R, /^@?[A-Z]+[a-z][A-Za-z_$@0-9]*/, null],
	...
	function Z(b) {
		function f(D) {
			if (D > r) {
				...
				var S = y(p(i.substring(r, D))).replace(e ? d : c, "$1&#160;");
				e = k.test(S);
				n.push(S.replace(a, g));
				r = D
			}
		}
		var i = b.source, o = b.extractedTags, l = b.decorations, n = [], r = 0, j = null, q = null, m = 0, t = 0, p = X(window.PR_TAB_WIDTH), c = /([\r\n ]) /g, d = /(^| ) /gm, a = /\r\n?|\n/g, k = /[ \r\n]$/, e = true;
		window._pr_isIE6();
		var h = b.sourceNode.className.match(/\blinenums\b(?::(\d+))?/), g;
		if (h) {
			for (var s = [], u = 0; u < 10; ++u)
				s[u] = '<br /></li><li class="L' + u + '">';
			var w = h[1] && h[1].length ? h[1] - 1 : 0;
			n.push('<ol class="linenums"><li class="L', w % 10, '"');
			w && n.push(' value="', w + 1, '"');
			n.push(">");
			g = function() {
				var D = s[++w % 10];
	...
	function P(b, f) {
		b && F.hasOwnProperty(b)
				|| (b = /^\s*</.test(f) ? "default-markup" : "default-code");
		return F[b]
	}
	function T(b) {
		var f = b.sourceCodeHtml, i = b.langExtension;
					...
					if (m.length > 1 && m.charAt(0) === "<") {
						if (!aa.test(m))
							if (ba.test(m)) {
								...
								f.push("\n");
								++n
							} else if (m.indexOf(U) >= 0
									&& m
											.replace(
													/\s(\w+)\s*=\s*(?:\"([^\"]*)\"|'([^\']*)'|(\S+))/g,
													' $1="$2$3$4"')
											.match(/[cC][lL][aA][sS][sS]=\"[^\"]*\bnocode\b/)) {
								var t = m.match(V)[2], p = 1, c;
								c = j + 1;
								a : for (; c < q; ++c) {
									var d = l[c].match(V);
									if (d && d[2] === t)
						...
						if (k < 0)
							...
							a = p.replace(da, "<").replace(ea, ">").replace(fa,
									"'").replace(ga, '"').replace(ha, " ")
									.replace(ia, "&")
						}
						f.push(a);
						n += a.length
	...
	var A = "str", Q = "kwd", C = "com", R = "typ", I = "lit", E = "pun", z = "pln", O = "src", U = "nocode", Y = function() {
		for (var b = ["!", "!=", "!==", "#", "%", "%=", "&", "&&", "&&=", "&=",
				"(", "*", "*=", "+=", ",", "-=", "->", "/", "/=", ":", "::",
				";", "<", "<<", "<<=", "<=", "=", "==", "===", ">", ">=", ">>",
				">>=", ">>>", ">>>=", "?", "@", "[", "^", "^=", "^^", "^^=",
				...
				"throw", "try", "typeof"], f = "(?:^^|[+-]", i = 0; i < b.length; ++i)
			f += "|" + b[i].replace(/([^=<>:&a-z])/g, "\\$1");
		f += ")\\s*";
		return f
	}(), K = /&/g, L = /</g, M = />/g, W = /\"/g, da = /&lt;/g, ea = /&gt;/g, fa = /&apos;/g, ga = /&quot;/g, ia = /&amp;/g, ha = /&nbsp;/g, ja = /[\r\n]/g, J = null, $ = RegExp(
			"[^<]+|<!--[\\s\\S]*?--\>|<!\\[CDATA\\[[\\s\\S]*?\\]\\]>|</?[a-zA-Z](?:[^>\"']|'[^']*'|\"[^\"]*\")*>|<",
			"g"), aa = /^<\!--/, ba = /^<!\[CDATA\[/, ca = /^<br\b/i, V = /^<(\/?)([a-zA-Z][a-zA-Z0-9]*)/, ka = x(
			{
	...
	window.prettyPrintOne = function(b, f) {
		...
		T(i);
		return i.prettyPrintedHtml
	};
	window.prettyPrint = function(b) {
		function f() {
			for (var t = window.PR_SHOULD_USE_CONTINUATION
					? j.now() + 250
				...
				PR
						.createSimpleLexer(
								...
								[[PR.PR_KEYWORD,
										/^-?(?:[_a-z]|(?:\\[\da-f]+ ?))(?:[_a-z\d\-]|\\(?:\\[\da-f]+ ?))*/i]]),
				["css-kw"]);
PR.registerLangHandler(PR
				.createSimpleLexer([], [[PR.PR_STRING, /^[^\)\"\']+/]]),
		["css-str"]);
