...
function HTML2Markdown(html, opts) {
	...
	try {
		...
		HTMLParser(dom, {
			start : function(tag, attrs, unary) {
				...
				switch (tag) {
					...
					case "img" :
						...
						attribs['alt']
								? alt = attribs['alt'].value.trim()
								: alt = "";
						attribs['title'] ? title = attribs['title'].value
								.trim() : title = "";

						// if parent of image tag is nested in anchor tag use
						// inline style
						if (!inlineStyle
								&& !peekTillNotEmpty(nodeList).startsWith("[")) {
							...
							// if image is not a link image then treat images as
							// block elements
			...
			"nodesToIgnore" : ["script", "noscript", "object", "iframe",
					"frame", "head", "style", "label"]
		});
