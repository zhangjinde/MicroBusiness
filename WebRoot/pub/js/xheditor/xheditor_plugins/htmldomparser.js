/*
 * HTMLParser - This implementation of parser assumes we are parsing HTML in
 * browser and user DOM methods available in browser for parsing HTML.
 * 
...
var HTMLParser = function(node, handler, opts) {
	...
	for (var i = 0; i < c.length; i++) {
		try {
			...
			if (c[i].nodeName.toLowerCase() != "#text"
					&& c[i].nodeName.toLowerCase() != "#comment") {
				var attrs = [];
				...
				if (handler.start) {
					if (c[i].hasChildNodes()) {
						...
						if (c[i].nodeName.toLowerCase() == "pre"
								|| c[i].nodeName.toLowerCase() == "code") {
							handler.chars(c[i].innerHTML);
						} else if (c[i].nodeName.toLowerCase() == "iframe"
								|| c[i].nodeName.toLowerCase() == "frame") {
							if (c[i].contentDocument
									&& c[i].contentDocument.documentElement) {
								return HTMLParser(
										c[i].contentDocument.documentElement,
										handler, opts);
							}
			...
			// properly log error
			console.log("error while parsing node: "
