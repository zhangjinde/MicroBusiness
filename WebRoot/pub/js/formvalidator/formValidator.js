...
(function($) {
	$.formValidator = {
		...
		initConfig : function(controlOptions) {
			...
			if (settings.formID != "") {
				$("#" + settings.formID).submit(function() {
					if (settings.ajaxForm != null) {
						$.formValidator.ajaxForm(settings.validatorGroup,
								settings.ajaxForm);
						return false;
					} else {
			...
			$.ajax({
						async : false,
						type : "GET",
						url : themedir + "themes/" + settings.theme
								+ "/js/theme.js",
						dataType : "script",
						error : function() {
		...
		sustainType : function(elem, validateType) {
			...
			switch (validateType) {
				...
				case "compareValidator" :
					return ((srcTag == "INPUT" || srcTag == "TEXTAREA")
							? (stype != "checkbox" && stype != "radio")
							: false);
				case "ajaxValidator" :
		...
		resetTipState : function(validatorGroup) {
			...
			$.each(initConfig.validObjects, function() {
						...
						$.formValidator.setTipState(this, passed
										? "onCorrect"
										: "onShow", passed
										? $.formValidator.getStatusText(this,
												setting.onCorrect)
										: setting.onShow);
					});
		},
			...
			if (!isValid) {
				...
				if (setting.validateType == "ajaxValidator") {
					...
					showmsg = (returnObj.errormsg == ""
							? $.formValidator.getStatusText(elem,
									setting.onError)
							: returnObj.errormsg);
			...
			$.each(initConfig.validObjects, function() {
				...
				if (this.settings[0].bind) {
					...
					if (name_list.indexOf("^" + name + "^") == -1) {
						onceValided = this.onceValided == undefined
								? false
								: this.onceValided;
						if (name) {
		...
		serialize : function(objs, initConfig) {
			...
			$.each(parmArray, function(index, data) {
						...
						if (li_pos > 0) {
							...
							var value = escape(decodeURIComponent(data
									.substr(li_pos + 1)));
							var parm = name + "=" + value;
		...
		passwordValid : function(elem) {
			...
			$.each(passwordStrengthRule, function(j, n) {
						...
						$.each(rules, function(i, rule) {
									...
									hasFind = true;
									$.each(getFlag(rule.flag), function(k,
													value) {
												if (value == 1) {
		...
		return this.each(function(e) {
			...
			if (initConfig.mode == "AutoTip" || initConfig.mode == "FixTip") {
				...
				if (initConfig.mode == "AutoTip" && tipDiv.length == 0) {
					var relativeID = setting_temp.relativeID
							? setting_temp.relativeID
							: id;
					var offset = $("#" + relativeID).offset();
					setting_temp.tipCss.top = offset.top
							+ setting_temp.tipCss.top;
					setting_temp.tipCss.left = $("#" + relativeID).width()
							+ offset.left + setting_temp.tipCss.left;
					tipDiv = $("<div style='position:absolute;' id='" + tip
							+ "'></div>").appendTo($("body"));
					tipDiv.css(setting_temp.tipCss);
					setting.relativeID = relativeID;
				}
				tipDiv.css("margin", "0px").css("padding", "0px").css(
			...
			setting.tipID = tip;
			setting.pwdTipID = setting_temp.pwdTipID
					? setting_temp.pwdTipID
					: setting.tipID;
			setting.fixTipID = setting_temp.fixTipID
					? setting_temp.fixTipID
					: id + "FixTip";
			...
			if (fixTip.length == 1 && onMouseOutFixTextHtml != ""
				...
				fixTip.css("padding", "0px 0px 0px 0px").css("margin",
						"0px 0px 0px 0px").html(onMouseOutFixTextHtml.replace(
						/\$data\$/g, setting.onShowFixText));
			}
			// 获取输入框内的提示内容
	...
	$.fn.ajaxValidator = function(controlOptions) {
		...
		return this.each(function() {
					...
					this.validatorAjaxIndex = $.formValidator.appendValid(
							this.id, settings);
				});
	};
	...
	$.fn.passwordValidator = function(controlOptions) {
		...
		$.extend(true, settings, passwordValidator_setting, controlOptions
						|| {});
		return this.each(function() {
					this.validatorPasswordIndex = $.formValidator.appendValid(
							this.id, settings);
				});
	};
		...
		return this.each(function() {
			...
			for (var i = 1; i < settings.length; i++) {
				...
				if (!($("body").data(settings[0].validatorGroup).mode == "AlertTip")) {
					...
					this.lastshowmsg = onShow
							? settings[0].onShow
							: settings[0].onCorrect;
					$.formValidator.setTipState(this, this.lastshowclass,
							this.lastshowmsg);
				}
			}
		});
	...
	$.fn.showTooltips = function() {
		...
		return this.each(function() {
			...
			this.tooltip = $("<span class='fv_tooltip' style='display:block'></span>")
