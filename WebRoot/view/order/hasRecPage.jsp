...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function orderFunc(){
				var row = $('#dg').datagrid('getSelected');
				if (row){
					$.messager.confirm('提示','您确定当前选中订单已完成吗?',function(r){
						if (r){
							$.post('/micro/order.do?method=orderFunc',{orderId:row.orderId,sts:'O'},function(result){
								if (result == "success"){
									$('#dg').datagrid('reload');	
								} else {
									$.messager.show({	
	...
	<body>
		<table id="dg" title="已发送订单" class="easyui-datagrid" width="100%" style="height:570px" url="<%=path %>/order.do?method=getOrderList&orderType=H" toolbar="#toolbar" pagination="true" rownumbers="true" fitColumns="true" singleSelect="true">
			<thead>
				<tr>
		...
		<div id="toolbar">
			...
			<a href="#" class="easyui-linkbutton" iconCls="icon-ok" plain="true" onclick="orderFunc()">完成订单</a>
